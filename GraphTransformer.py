...
from models import GraphTransformerMInterface
from dataloaders import GraphTransformerDInterface
...
def main(config):
    ...
    data_module = GraphTransformerDInterface(**vars(config))
    model = GraphTransformerMInterface(**vars(config))
...
if __name__ == "__main__":
    with open("config/graphtransformer.yaml", encoding="utf-8") as f:
        file = f.read()
    ...
    excel_file = config.osave_dir+f"/graph_transformer_results_{timestamp}.xlsx"
